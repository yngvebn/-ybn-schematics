...
export function story(options: any): Rule {
    return (tree: Tree, _context: SchematicContext) => {
        ...
        const rule = mergeWith(sourceParametrizedTemplates, MergeStrategy.AllowCreationConflict);
...
export function component(options: any): Rule {
    return (tree: Tree, _context: SchematicContext) => {
        ...
        const rule = mergeWith(sourceParametrizedTempalates, MergeStrategy.AllowCreationConflict);
        const { mdx, componentName, hasModule, sourceRoot, skipStories, componentFilename, ...allowedComponentOptions } = options;

        return chain([externalSchematic('@schematics/angular', 'component', allowedComponentOptions), story(options), rule]);
    };
}
